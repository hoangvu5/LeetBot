...
- ElevenLabs/Google TTS will generate the voice for the video and the timestamps for each word.
...
4. Set `OPENAI_API_KEY` and `XI_API_KEY` and `GOOGLE_API_KEY` in environment file `.env`.
...
2. Music: -10dB
3. Note that manim.cfg should be in the folder where rendering.py is run from.
